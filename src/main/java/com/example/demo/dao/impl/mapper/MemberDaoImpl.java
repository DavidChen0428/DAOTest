...
public class MemberDaoImpl implements DAO<Member>{
	...
	public List<Member> findSome(Object key) throws DAOException{
		if(key instanceof String) {
			if(existsByAddress((String)key)) {
				List<Member> list=memberMapper.findByAddress((String)key);
				if(list.size()==0) {
	...
	boolean existsByAddress(String key) {
		if(memberMapper.findByAddress(key).size()!=0) {
			return true;
		}else {
			return false;
		}
	...
	public Member findOne(Object key) throws DAOException {
		if (key instanceof Integer) {
			...
			return memberMapper.findById(keyValue).get(0);
		} else if (key instanceof String) {
			Member member=memberMapper.findByUsername((String)key).get(0);
			if(member==null) {
				member=memberMapper.findByName((String)key).get(0);
				if(member==null) {
	...
	public void delete(Object key) throws DAOException {
		if(key instanceof Integer) {
			memberMapper.deleteById((Integer) key);
		}else {
